...
public class VideoStore 
{
   ...
   /**
    ...
    public void setUpInventory(String videoFile) throws FileNotFoundException
    {
    	...
    	while(s.hasNextLine())
    	{
    		String filmLine = s.nextLine();
    		String film = parseFilmName(filmLine);
    		int copies = parseNumCopies(filmLine);
    		if(findVideo(film)==null)
    		{
    			Video v = new Video(film,copies);
        		inventory.addBST(v);
    		}
    		else
    		{
    			Video v = findVideo(film);
    			v.addNumCopies(copies);
    		}
    	}
    	s.close();
    }
	...
	{
		Iterator<Video> v = inventory.iterator();
		while(v.hasNext())
		{
			Video vid = v.next();
			if(vid.getFilm().equals(film))
			{
				return vid;
			}
		}
	...
	public String inventoryList()
	{
		...
		String s = "";
		System.out.println("Films in inventory: ");
		System.out.println("");
		while(iter.hasNext())
		{
			Video f = iter.next();
			s+=f.getFilm()+" ("+f.getNumCopies()+")";
			s+="\n";
		}
		return s;
	}
	...
	private String rentedVideosList()
	{
		...
		{
			Video f = iter.next();
			if(f.getNumRentedCopies()>0)
			{
				s+=f.getFilm()+" ("+f.getNumRentedCopies()+")";
				s+="\n";
			}
		}
		return s;
	...
	private String unrentedVideosList()
	{
		...
		{
			Video f = iter.next();
			if(f.getNumAvailableCopies()>0)
			{
				s+=f.getFilm()+" ("+f.getNumAvailableCopies()+")";
				s+="\n";
			}
		}
		return s; 
	...
	public static String parseFilmName(String line) 
	{
		...
		{
			String next = s.next();
			if(next.charAt(0)=='('&&Character.isDigit(next.charAt(1)))
			{
				stop=false;
			}
			else
